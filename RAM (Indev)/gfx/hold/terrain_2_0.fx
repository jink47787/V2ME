...
float4 PixelShader_Map2_0_General_Low( VS_MAP_OUTPUT v ) : COLOR
{
    ...
    // Generate the terrain color
    float4 TerrainColor = GenerateTiles( s );

    // You can adjust the intensity of the terrain color if needed
    TerrainColor.rgb *= 1.2; // Increase brightness slightly

    // Optional: Blend with overlay for additional detail
    float4 OverlayColor = tex2D( OverlayTexture, v.vColorTexCoord );
    TerrainColor = lerp(TerrainColor, OverlayColor, 0.3);
