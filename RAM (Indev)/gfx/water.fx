...
float4 PixelShader_Far( VS_OUTPUT_WATER_FAR IN ) : COLOR
{
    ...
    // Define a lighter, more washed-out parchment color
    float3 parchmentColor = float3(0.95, 0.93, 0.85);
    
    // Blend more heavily towards the parchment color for a more washed-out look. 0.8 to 0.75
    float3 blendedColor = lerp(baseColor.rgb, parchmentColor, 0.75);
    
    // Slightly desaturate the color, from 0.2 to 0.3
    float luminance = dot(blendedColor, float3(0.299, 0.587, 0.114));
    blendedColor = lerp(blendedColor, luminance.xxx, 0.2);
    
    // Brighten the color slightly. from 1.1, 0.9
    blendedColor = saturate(blendedColor * 1.05);	
    
    // Calculate edge darkening only for the bottom edge (very subtle now)
    float bottomEdge = IN.vUV.y;
    float edgeDarkening = lerp(0.97, 1.0, saturate(bottomEdge * 15));
    
    // Apply edge darkening
    float3 finalColor = blendedColor * edgeDarkening;
    
    // Add a very slight warm tone
    finalColor.r += 0.03;
    finalColor.g += 0.02;
    finalColor.b += 0.01;
